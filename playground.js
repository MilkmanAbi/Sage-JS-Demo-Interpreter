/* =========================================================================
   ...
   Traffic lights are wired per the brief:
     red  → close/quit (tear the app down; relaunch fresh from the dock).
            The editor asks first if it holds unsaved buffers.
     yellow → minimise (genie to the dock; minimise PRESERVES state)
...
(function () {
  ...
  function wireChrome(app) {
    ...
    if (rDot) rDot.addEventListener('click', function (e) { e.stopPropagation(); requestClose(app); });
  ...
  function resetApp(app) {
    if (!app) return;
    var win = WINS[app.id];
    if (win.closed) { openApp(app); return; }   // a quit app "resets" by relaunching
  ...
  /* ---- close / open (red light genuinely quits; dock relaunches) ------ */
  // The red light asks first when there's unsaved work in the editor.
  function requestClose(app) {
    if (app.id === 'editor') {
      var ed = WINS.editor.inst;
      if (ed && typeof ed.hasUnsaved === 'function' && ed.hasUnsaved()) {
        promptCloseEditor(app);
        return;
      }
    }
    closeApp(app);
  ...
  // Quit the app: tear down its instance, wipe its content, hide the window,
  // and dim its dock dot. Reopening from the dock rebuilds it from scratch.
  function closeApp(app) {
    var win = WINS[app.id];
    if (win.closed) return;
    if (win.maxed) toggleMax(app, true);
    var r = win.el.getBoundingClientRect();
    fire({ x: r.left + r.width / 2, y: r.top + r.height / 2, count: 5 });
    if (win.inst && typeof win.inst.destroy === 'function') {
    ...
    // clear the content once the close transition has played
    setTimeout(function () {
      if (!win.closed) return;            // reopened in the meantime
      win.contentEl.innerHTML = '';
      win.contentEl.removeAttribute('class');
      win.contentEl.removeAttribute('style');
    }, 240);
    setDock(app.id, 'min', false);
    setDockOpen(app.id, false);
    if (focusedId === app.id) focusedId = null;
    revealDockBriefly();
  }

  // Relaunch a quit app: mount a fresh instance and float it back in.
  function openApp(app) {
    var win = WINS[app.id];
    if (!win.closed) { focusWin(app); return; }
    // ensure a clean content shell, then rebuild fresh state
    win.contentEl.innerHTML = '';
    win.contentEl.removeAttribute('class');
    win.contentEl.removeAttribute('style');
    buildApp(app);
    ...
    win.min = false;
    win.maxed = false;
    win.el.classList.remove('is-closed', 'is-min', 'is-max');
    setDock(app.id, 'min', false);
    setDockOpen(app.id, true);
    focusWin(app);
    flashReset(app);
  }
  function setDockOpen(id, val) {
    var item = document.querySelector('.pg-dock-item[data-app="' + id + '"]');
    if (item) item.classList.toggle('is-open', val);
  }
  // Put an app into the quit state instantly at boot (no animation, no fireflies).
  function bootClosed(app) {
    var win = WINS[app.id];
    if (win.inst && typeof win.inst.destroy === 'function') {
      try { win.inst.destroy(); } catch (e) {}
    }
    ...
    win.el.style.transition = 'none';
    win.el.classList.add('is-closed');
    void win.el.offsetHeight;            // commit the no-transition frame
    win.el.style.transition = '';
    setDockOpen(app.id, false);
  }

  // A soft, on-brand confirm sheet for closing the editor with unsaved buffers.
  function promptCloseEditor(app) {
    var ed = WINS.editor.inst;
    var n = (ed && ed.unsavedCount && ed.unsavedCount()) || 0;
    var noun = n === 1 ? 'file that has' : 'files that have';
    closeModal();
    var veil = document.createElement('div');
    veil.className = 'pg-modal-veil';
    veil.innerHTML =
      '<div class="pg-modal pg-confirm" role="dialog" aria-modal="true">' +
        '<h3><span class="glyph"><i data-lucide="file-warning"></i></span>Close the editor?</h3>' +
        '<p class="sub">You have <strong>' + n + '</strong> ' + noun + ' never been saved to the project. ' +
          'Closing resets the editor — unsaved buffers will be lost unless you keep them first.</p>' +
        '<div class="pg-confirm-actions">' +
          '<button class="pg-btn pg-btn-primary" data-act="save"><i data-lucide="hard-drive-download"></i>Save to project</button>' +
          '<button class="pg-btn" data-act="download"><i data-lucide="download"></i>Download…</button>' +
          '<button class="pg-btn pg-btn-danger" data-act="discard"><i data-lucide="trash-2"></i>Close without saving</button>' +
          '<button class="pg-btn pg-btn-ghost" data-act="cancel">Cancel</button>' +
        '</div>' +
      '</div>';
    ...
    function act(name) {
      if (name === 'save')     { if (ed && ed.saveAllToVFS) ed.saveAllToVFS(); closeModal(); closeApp(app); }
      else if (name === 'download') { if (ed && ed.downloadUnsaved) ed.downloadUnsaved(); /* keep sheet open so they can still save/close */ }
      else if (name === 'discard')  { closeModal(); closeApp(app); }
      else { closeModal(); }
    }
    veil.querySelectorAll('.pg-btn').forEach(function (b) {
      b.addEventListener('click', function (e) { e.stopPropagation(); act(b.dataset.act); });
    });
  }
  ...
  function buildDock() {
    ...
    APPS.forEach(function (app) {
      ...
      item.addEventListener('click', function () {
        var win = WINS[app.id];
        if (win.closed) openApp(app);
        else if (win.min) restore(app);
        else focusWin(app);
  ...
  /* ===================================================================
     ...
     =================================================================== */
  function focusApp(id) { var a = appById(id); if (WINS[id].closed) openApp(a); else if (WINS[id].min) restore(a); else focusWin(a); }
  function restoreApp(id) { var a = appById(id); if (WINS[id].closed) openApp(a); else if (WINS[id].min) restore(a); }
  // The app the Window menu acts on: the focused one, or the first still-open window.
  function focusedApp() {
    var a = appById(focusedId);
    if (a && !WINS[a.id].closed) return a;
    for (var i = 0; i < APPS.length; i++) { if (!WINS[APPS[i].id].closed) return APPS[i]; }
    return null;
  }
  ...
  function syncEditor() {
    if (WINS.editor && WINS.editor.closed) openApp(appById('editor'));
    var ed = inst('editor');
  ...
  function resetLayout() {
    APPS.forEach(function (a) {
      var w = WINS[a.id];
      if (w.closed) return;          // leave quit apps quit
  ...
  // ---- Run ----
  function runEditor() { restoreApp('editor'); focusApp('editor'); var ed = inst('editor'); if (ed && ed._run) { ed._run(); fire({ count: 5 }); } }

  // ---- Window ----
  function bringAllFront() { APPS.forEach(function (a) { if (!WINS[a.id].closed && WINS[a.id].min) restore(a); }); focusApp('editor'); }
  ...
  function showAbout() {
    openModal(
      ...
      '<p>The traffic lights work: <span class="k">red</span> closes an app (reopen it from the dock, fresh), <span class="k">yellow</span> tucks it to the dock, <span class="k">green</span> fills the screen. Click the dark to stir the fireflies.</p>' +
  ...
  var MENUS = [
    ...
    { name: 'Run', items: [
      ...
      { sep: true },
      { label: 'Clear Output', act: clearOutput },
      { label: 'Restart REPL', act: function () { var r = appById('repl'); if (WINS.repl.closed) openApp(r); else resetApp(r); focusApp('repl'); } }
    ] },
    { name: 'Window', items: [
      { label: 'Minimize', kbd: '\u2318M', act: function () { var a = focusedApp(); if (a) minimise(a); } },
      { label: 'Zoom / Maximize', act: function () { var a = focusedApp(); if (a) toggleMax(a); } },
      { label: 'Reset This App', act: function () { var a = focusedApp(); if (a) resetApp(a); } },
      { sep: true },
  ...
  function startShortcuts() {
    window.addEventListener('keydown', function (e) {
      ...
      else if (k === 'm') { e.preventDefault(); var a = focusedApp(); if (a) minimise(a); }
      else if (k === 'enter') {
  ...
  function initWindows() {
    ...
    APPS.forEach(function (app) {
      ...
      wsEl.appendChild(el);
      WINS[app.id] = { el: el, contentEl: content, min: false, maxed: false, closed: false, userMoved: false };
    });
    ...
    layout();
    // The Files window starts closed so the default 3-pane layout stays clean —
    // give it a sensible centred restore size first, then quit it to the dock.
    (function setupFiles() {
      ...
      bootClosed(appById('files'));
    })();
    // Notes also starts closed; open it from the dock when you want it.
    (function setupNotes() {
  ...
  function init() {
    ...
    window.SagePlayground = {
      openInEditor: function (path) {
        focusApp('editor');                 // reopens if closed, restores if minimised
        var ed = inst('editor');
