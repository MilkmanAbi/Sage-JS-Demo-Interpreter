...
(function(global) {
  ...
  function cmdInit(state, args, out) {
    ...
    state.installed = {};
    state.lockfile = {};
    // Pick a few "already in source" imports for scan simulation
    const allPkgs = Object.keys(REGISTRY);
    state.sourceImports = [];
  ...
  function cmdAdd(state, args, out) {
    ...
    const { resolved, error } = resolveAll(pkgArgs, state.installed);
    ...
    for (const [name, ver] of resolved.entries()) {
      if (!state.installed[name]) {
        const isDirect = pkgArgs.some(p => parseVer(p).name === name);
    ...
    for (const { name, ver, isDirect } of toInstall) {
      const entry  = REGISTRY[name];
      const verData = entry.versions[ver];
    ...
    // Install order: deps before dependents (rough topo by iterating resolved)
    const installOrder = [];
    const added = new Set();
    ...
    for (const { name, ver, isDirect } of toInstall) {
      const verData = REGISTRY[name].versions[ver];
      state.installed[name] = { version: ver, size_kb: verData.size_kb, transitive: !isDirect };
      state.lockfile[name]  = { version: ver, hash: sha256stub(name, ver) };
      if (isDirect) {
        state.manifest[name] = ver;
        if (isDev) state.installed[name].dev = true;
      }
      // Also add direct deps to manifest as transitive entries
    }

    out('  installed  ' + toInstall.length + ' package' + (toInstall.length>1?'s':'') +
        '  ·  ' + fmtSize(totalKB) + '  ·  lockfile updated', 'ok');

    // Warn about health issues
  ...
  function _doRemove(state, pkgName, orphans, out) {
    ...
    out('  removed  ' + total + ' package' + (total>1?'s':'') + '  ·  lockfile updated', 'ok');
    out('');
  }
  ...
  function cmdHealth(state, args, out) {
    ...
    for (const [name, pkg] of pkgs.sort(([a],[b])=>a.localeCompare(b))) {
      const verData = REGISTRY[name]?.versions[pkg.version];
      const grade   = verData?.health || 'A';
      const notes   = verData?.notes  || 'all clear';
    ...
    const issues = pkgs.filter(([n,p])=>{const g=REGISTRY[n]?.versions[p.version]?.health;return g&&g!=='A';}).length;
  ...
  function cmdGraph(state, args, out) {
    ...
    function renderTree(pkgName, version, prefix, isLast, visited) {
      ...
      visited.add(pkgName);
      out(prefix + (isLast?'└──':'├──') + ' ' + pkgName + ' ' + version);
      const verData = REGISTRY[pkgName]?.versions[version];
      if (!verData) return;
      const deps = verData.deps.filter(d=>state.installed[parseVer(d).name]);
  ...
  // ── Command dispatcher ────────────────────────────────────────────────────
  function dispatch(state, input, out) {
    ...
    if (state.pendingRemove) {
      ...
      if (ans === 'y' || ans === 'yes') {
        _doRemove(state, pkgName, orphans, out);
      } else {
        _doRemove(state, pkgName, [], out);
      }
      return;
    }

    // Strip leading "myst " if user types the full command
    ...
    switch(cmd) {
      ...
      case 'add':        cmdAdd(state, args, out); break;
      ...
      default:
        ...
        const CMDS = ['init','add','install','remove','update','list','status','health','scan','duplicates','graph','lock','search','version','help'];
  ...
  class MystTerminal {
    ...
    _tabComplete() {
      ...
      if (['add','search'].includes(cmd)) {
        ...
        const matches = Object.keys(REGISTRY).filter(p=>p.startsWith(prefix));
        if (matches.length === 1) {
    ...
    _submit(raw) {
      ...
      // Dispatch
      dispatch(this.state, line, (text, kind='default') => {
        if (text === '\x1b[2J') { this._clear(); return; }
        this._line(text, kind);
      });

      // Update chrome project label
