...
(function (global) {
  ...
  class Lexer {
    ...
    _push(type, value, line, col) {
      // Blank lines and comment-only lines would otherwise emit a NEWLINE with
      // no statement before it, producing NEWLINE-NEWLINE runs. A stray NEWLINE
      // right after a block-opening ':' lands before the INDENT and breaks the
      // block parser. Collapse consecutive NEWLINEs to a single separator.
      if (type === TT.NEWLINE) {
        const last = this.tokens[this.tokens.length - 1];
        if (!last || last.type === TT.NEWLINE) return;
      }
  ...
  class Parser {
    ...
    _procDef() {
      ...
      // proc/method names may shadow keyword-like tokens (var, type, in, …):
      // accept any token whose value is a valid identifier.
      let name;
      const nt = this._cur();
      if (nt.type === TT.IDENT) name = this._adv().value;
      else if (typeof nt.value === 'string' && /^[a-zA-Z_]\w*$/.test(nt.value)) name = this._adv().value;
      else name = this._eat(TT.IDENT, 'expected proc name').value;
      this._eat(TT.LPAREN);
      const params = [];
    ...
    _importStmt() {
      ...
      // hyphenated package names: sage-numpy, lilybox-core, …
      while (this._check(TT.MINUS) && this._peek(1).type === TT.IDENT) {
        this._adv();                                  // consume '-'
        module += '-' + this._adv().value;
      }
      // dotted submodules: torch.nn.functional
      while (this._match(TT.DOT)) module += '.' + this._eat(TT.IDENT).value;
      // optional alias:  import sage-numpy as np
      let alias = null;
      if (this._check(TT.IDENT) && this._cur().value === 'as') {
        this._adv();
        alias = this._eat(TT.IDENT).value;
      }
      this._match(TT.NEWLINE);
      return { type:'Import', module, alias, line:kw.line };
    }
    ...
    _postfix() {
      ...
      while (true) {
        if (this._check(TT.DOT)) {
          const op=this._adv();
          // attribute names may be identifiers OR keyword-like tokens
          // (python.import, obj.type, x.in, …) — accept any token that
          // carries a string value and treat it as a member name.
          let name;
          const t = this._cur();
          if (t.type === TT.IDENT) {
            name = this._adv().value;
          } else if (typeof t.value === 'string' && /^[a-zA-Z_]\w*$/.test(t.value)) {
            name = this._adv().value;
          } else {
            name = this._eat(TT.IDENT, 'expected attribute name').value;
          }
  ...
  class Evaluator {
    constructor(outputFn, opts={}) {
      ...
      this.cwd = (opts && opts.cwd) || '/project';   // where myst_libs lives
      this.vfs = (opts && opts.vfs) || global.SageVFS || null;
      this.moduleCache = Object.create(null);
      this._stubNoted  = Object.create(null);
    }
    ...
    _setupBuiltins(env) {
      ...
      // let emulated FFI backends (sage-ffi.js) invoke Sage callables, e.g.
      // series.apply(proc(x): …). Set fresh each run to the active evaluator.
      if (global.SageFFI) {
        global.SageFFI.__call = (fn, args) => self._callValue(fn, Array.isArray(args) ? args : [args]);
        global.SageFFI.__dict = (pairs) => ({__type:'dict', data:new Map(pairs)});
      }
      ...
      env.define('list',    bi((it)=>{ if(it==null)return []; if(Array.isArray(it))return it.slice(); if(typeof it==='string')return it.split(''); if(it&&it.__type==='dict')return [...it.data.keys()]; if(it&&it.__type==='tuple')return it.items.slice(); if(it&&typeof it[Symbol.iterator]==='function')return [...it]; return [it]; }));
      ...
      const runSandboxed = (code) => {
        const lines = [];
        try {
          const sub = new Evaluator((t)=>lines.push(t), { cwd: self.cwd, vfs: self.vfs });
          sub.stepLimit = 200000;   // CPU cap — sandboxed code can't run forever
          sub.steps = 0;
          sub.evalSource(code);
          return { ok:true, stdout: lines.join('\n'), error:null };
        } catch (e) {
          const msg = (e && e.value !== undefined) ? sageStr(e.value) : (e && e.message) || String(e);
      ...
      const sandbox = {
        ...
        // resource limits & permissions — recorded, return the box for chaining
        set_memory_cap: bi((box,mb)=>{ self.output(`  LilyBox ✦  memory cap ${mb} MB`); return box; }),
        set_cpu_cap:    bi((box,ms)=>{ self.output(`  LilyBox ✦  cpu cap ${ms} ms`); return box; }),
        deny_network:   bi((box)=>{ self.output('  LilyBox ✦  network denied'); return box; }),
        deny_fs:        bi((box)=>{ self.output('  LilyBox ✦  filesystem denied'); return box; }),
        grant_read:     bi((box,path)=>{ self.output(`  LilyBox ✦  granted read: ${path}`); return box; }),
        // execution — actually runs the Sage code inside a fresh evaluator
        eval_code: bi((box,code)=>{ self.output('  LilyBox ✦  eval_code (sandboxed)'); return runSandboxed(code); }),
        run: bi((box,file,...args)=>{
          self.output(`  LilyBox ✦  running '${file}' inside sandbox`);
          const src = self.vfs ? self.vfs.read(self.vfs.join(self.cwd, file)) : null;
          if (src == null) return { ok:false, stdout:'', error:`file not found: ${file}` };
          return runSandboxed(src);
        }),
        run_with_input: bi((box,file,input)=>{
          self.output(`  LilyBox ✦  running '${file}' with input`);
          const src = self.vfs ? self.vfs.read(self.vfs.join(self.cwd, file)) : null;
          if (src == null) return { ok:false, stdout:'', error:`file not found: ${file}` };
          return runSandboxed(src);
        }),
        send: bi((box,msg)=>{ self.output(`  LilyBox ✦  message → sandbox`); return { ok:true, stdout: sageStr(msg), error:null }; }),
      ...
      // python FFI stub — routes to an emulated backend (sage-ffi.js) when one
      // is registered, so installed packages actually compute instead of echoing.
      const python = {
        __type:'module', name:'python',
        import: bi((name)=>{
          const reg = global.SageFFI && global.SageFFI.python && global.SageFFI.python[name];
          if (reg) {
            self.output(`  Python FFI ✦  import '${name}' (emulated backend)`);
            return new Proxy(reg, { get(t, prop){
              const key = String(prop);
              const v = t[key];
              if (typeof v === 'function') return {__type:'builtin', fn:(...a)=>v.apply(t, a)};
              if (v !== undefined) return v;                       // constants like pi, inf
              return {__type:'builtin', fn:(...a)=>`${name}.${key}(${a.map(sageStr).join(', ')})`};
            }});
          }
      ...
      // C FFI stub — routes bound symbols to an emulated C library (sage-ffi.js)
      // when one is registered (e.g. hiredis, libcurl); echoes otherwise.
      const cHelp = (name)=> (global.SageFFI && global.SageFFI.cHelpers && global.SageFFI.cHelpers[name]) || null;
      const c_ffi = {
        __type:'module', name:'c_ffi',
        load:   bi((lib)=>{
          const impl = global.SageFFI && global.SageFFI.c && global.SageFFI.c[lib];
          if (impl) { self.output(`  C FFI ✦  loaded '${lib}' (emulated backend)`); return {__type:'clib', name:lib, _impl:impl}; }
          self.output(`  C FFI ✦  loaded '${lib}' (emulated)`); return {__type:'clib', name:lib, _impl:null};
        }),
        bind:   bi((lib, sym, ..._)=>{
          const impl = lib && lib._impl;
          if (impl && typeof impl[sym] === 'function') {
            return {__type:'builtin', fn:(...args)=>impl[sym].apply(impl, args)};
          }
          return {__type:'builtin', fn:(...args)=>{ self.output(`  C FFI ✦  ${sym}(${args.map(sageStr).join(', ')}) = (emulated)`); return 0; }};
        }),
        unload: bi((lib)=>{ self.output(`  C FFI ✦  unloaded '${lib.name}'`); return null; }),
        // reply / context helpers used by C-backed libs (redis, requests, …)
        reply_str:  bi((r)=> cHelp('reply_str')  ? cHelp('reply_str')(r)  : sageStr(r)),
        reply_int:  bi((r)=> cHelp('reply_int')  ? cHelp('reply_int')(r)  : 0),
        reply_array:bi((r)=> cHelp('reply_array')? cHelp('reply_array')(r): []),
        reply_type: bi((r)=> cHelp('reply_type') ? cHelp('reply_type')(r) : 'nil'),
        reply_is_nil:bi((r)=>cHelp('reply_is_nil')? cHelp('reply_is_nil')(r): 1),
        ctx_err:    bi((c)=> cHelp('ctx_err')    ? cHelp('ctx_err')(c)    : 0),
        ctx_errmsg: bi((c)=> cHelp('ctx_errmsg') ? cHelp('ctx_errmsg')(c) : ''),
        curl_status:bi((h)=> cHelp('curl_status')? cHelp('curl_status')(h): 0),
        curl_body:  bi((h)=> cHelp('curl_body')  ? cHelp('curl_body')(h)  : ''),
      };
    ...
    _execStmt(node, env) {
      if (!node) return null;
      // opt-in execution budget — only set on sandboxed sub-evaluators, so the
      // main interpreter is unaffected. Lets LilyBox CPU caps actually bite.
      if (this.stepLimit !== undefined && ++this.steps > this.stepLimit) {
        throw new RaiseSignal('ResourceLimit: CPU/step cap exceeded');
      }
      switch(node.type) {
    ...
    _execImport(node, env) {
      ...
      // default binding name when no explicit alias: last path segment,
      // de-hyphenated so it's a usable identifier (sage-numpy -> numpy)
      const bind = node.alias || mod.split(/[-.]/).filter(Boolean).pop();

      // 1. built-in FFI / stdlib stubs already in globals
      if (mod === 'python' || mod === 'c_ffi' || mod === 'sandbox' || mod === 'math') {
        const m = this.globalEnv.get(mod);
        if (m !== undefined) { env.define(bind, m); return m; }
      }

      // 2. a real package installed in the VFS (Myst put it there)
      const loaded = this._loadVfsModule(mod);
      if (loaded) { env.define(bind, loaded); return loaded; }

      // 3. graceful fallback — a permissive stub so unresolved imports
      //    (e.g. a transitive dep that wasn't fetched) don't crash a lesson
      const stub = this._stubModule(mod);
      env.define(bind, stub);
      return stub;
    }

    // Load a Sage package from the virtual filesystem and evaluate its source,
    // returning a module object whose members are its top-level definitions.
    _loadVfsModule(name) {
      if (this.moduleCache[name]) return this.moduleCache[name];
      const vfs = this.vfs || global.SageVFS;
      if (!vfs) return null;

      const base = this.cwd + '/myst_libs/' + name;
      if (!vfs.exists(base)) return null;

      // entry point: prefer src/lib.sage, then lib.sage, then src/<name>.sage
      const candidates = [
        base + '/src/lib.sage',
        base + '/lib.sage',
        base + '/src/' + name + '.sage',
      ...
      let entry = null;
      for (const c of candidates) { if (vfs.isFile(c)) { entry = c; break; } }
      if (!entry) return null;

      const src = vfs.read(entry);
      const modEnv = new Env(this.globalEnv);

      // pre-register in the cache to tolerate self/circular references
      const moduleObj = { __type: 'sage_module', name, env: modEnv };
      this.moduleCache[name] = moduleObj;

      try {
        const lexer  = new Lexer(src);
        const parser = new Parser(lexer.tokens);
        const ast    = parser.parse();
        this._execBlock(ast.body, modEnv);
      } catch (e) {
        // surface a clear, non-fatal note and fall back to a stub
        const msg = (e && e.value !== undefined) ? sageStr(e.value) : (e && e.message) || String(e);
        this.output(`  import \u2726  '${name}' loaded with warnings: ${msg}`);
      }
      return moduleObj;
    }

    // A permissive module stub: any member is a callable that echoes an
    // emulated result. Lets unresolved imports keep a lesson moving.
    _stubModule(name) {
      const self = this;
      if (!this._stubNoted[name]) {
        this.output(`  import \u2726  '${name}' is not installed \u2014 using an emulated stub`);
        this._stubNoted[name] = true;
      }
      return { __type: 'stub_module', name };
    }
    ...
    _evalAttr(node, env) {
      ...
      // a Sage package loaded from the VFS — members are its top-level defs
      if (obj.__type==='sage_module') {
        const v = obj.env.get(node.name);
        if (v !== undefined) return v;
        // unknown member — soft stub so chained calls still resolve
        return {__type:'builtin',fn:(...a)=>{ this.output(`  ${obj.name}.${node.name}(${a.map(sageStr).join(', ')}) \u2014 (not exported)`); return null; }};
      }
      // permissive stub module — every member is an emulated callable
      if (obj.__type==='stub_module') {
        const modName = obj.name;
        const self = this;
        return {__type:'builtin',fn:(...a)=>{ self.output(`  ${modName}.${node.name}(${a.map(sageStr).join(', ')}) \u2014 (emulated)`); return null; }};
      }
      // module
      ...
      // plain JS objects and FFI proxies (python.import(...) returns a Proxy)
  ...
  // expose the front-end for tooling (file browser preview, linting, etc.)
  global.SageLang = { Lexer, Parser, Evaluator, Env, sageStr, sageBool };
