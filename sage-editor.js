...
`,
  ...
  function injectCSS() {
    ...
    /* unsaved (in-memory only) buffer: a bright dot signals it isn't on disk yet */
    .se-tab.is-unsaved .se-tab-dot { background:var(--accent); opacity:1; box-shadow:0 0 0 3px rgba(160,104,216,.18); }
  ...
  class SageEditor {
    constructor(container) {
      ...
      this.filePaths   = {};   // tab name -> VFS path (for files backed by the filesystem)

      // Seed the open tabs from the shared filesystem when present; otherwise
      // fall back to the built-in sample files (standalone usage).
      if (this.vfs) {
        const seeded = this._seedFromVFS();
        this.fileContents = seeded.contents;
        this.filePaths    = seeded.paths;
        this.currentFile  = seeded.first;
      } else {
      ...
      // open-file requests routed from the file browser / playground bridge
      const self = this;
      this._openListener = function (e) {
        if (e && e.detail && e.detail.path) self.openInEditor(e.detail.path);
      };
      global.addEventListener('sage:open-file', this._openListener);

      // Reconcile with the shared filesystem when it changes elsewhere (e.g. a
      // file is deleted in the file browser). A tab whose backing file vanished
      // becomes a plain in-memory buffer again — still closable, still re-saveable
      // — instead of a phantom pointing at a dead path.
      if (this.vfs && this.vfs.on) {
        this._offVfs = this.vfs.on(() => this._syncWithVFS());
      }
    }

    // Drop VFS mappings whose files no longer exist; mark those tabs unsaved.
    _syncWithVFS() {
      if (!this.vfs || !this.filePaths) return;
      if (this._savingToVFS) return;          // ignore the echo of our own write
      Object.keys(this.filePaths).forEach(name => {
        const p = this.filePaths[name];
        if (p && !this.vfs.isFile(p)) {
          delete this.filePaths[name];
          this._setTabSaved(name, false);     // becomes an unsaved in-memory buffer
        }
      });
    }

    // Read top-level .sage / .sageh files from the project dir into tabs.
    _seedFromVFS() {
      const PROJ = this.vfs.PROJECT || '/project';
      const contents = {};
      const paths = {};
      let first = null;
      const entries = this.vfs.ls(PROJ).filter(e =>
        e.type === 'file' && /\.(sage|sageh)$/.test(e.name));
      // put main.sage first if present
      entries.sort((a, b) => (a.name === 'main.sage' ? -1 : b.name === 'main.sage' ? 1 : a.name.localeCompare(b.name)));
      entries.forEach(e => {
        const name = this._dedupe(e.name, contents);
        contents[name] = this.vfs.read(e.path) || '';
        paths[name] = e.path;
        if (!first) first = name;
      });
      if (!first) {
        // empty project — start an untitled buffer that lives only in memory.
        // It is NOT written to the filesystem until the user names it (rename)
        // or runs it, so an unused sketch never litters the project dir.
        first = 'main.sage';
        contents[first] = '# a fresh sage sketch  \u2726\n\nproc main():\n    println("hello, sage")\n\nmain()\n';
        // intentionally no paths[first] — unmapped == unsaved/in-memory
      }
      return { contents, paths, first };
    }
    _dedupe(name, dict) {
      ...
      let i = 1, n;
      do { n = stem + '-' + (i++) + ext; } while (dict[n] !== undefined);
      return n;
    }
    ...
    _build() {
      ...
      tabs.id = 'se-tabs';
      this.tabsEl = tabs;
      Object.keys(this.fileContents).forEach(name => {
    ...
    _makeTab(name) {
      ...
      // a buffer with no filesystem home yet is "unsaved" — shown with a dot
      if (this.vfs && !(this.filePaths && this.filePaths[name])) {
        tab.classList.add('is-unsaved');
      }
      tab.innerHTML =
        `<span class="se-tab-dot" title="unsaved — rename the tab or run to save"></span>` +
    ...
    // Toggle the unsaved (in-memory) indicator on a tab.
    _setTabSaved(name, saved) {
      const tab = this._tabFor(name);
      if (tab) tab.classList.toggle('is-unsaved', !saved);
    }
    ...
    _applyRename(tab, oldName, newName) {
      ...
      if (newName !== oldName) {
        ...
        // keep the filesystem mapping in step (rename the VFS file too).
        // Naming a buffer is also its FIRST save: if it had no file yet, this
        // is where it lands in the project for the first time.
        if (this.vfs && this.filePaths) {
          const oldPath = this.filePaths[oldName];
          delete this.filePaths[oldName];
          if (oldPath && this.vfs.isFile(oldPath)) {
            try {
              const newPath = this.vfs.rename(oldPath, newName);
              this.filePaths[newName] = newPath;
            } catch (e) {
              // name clash etc. — fall back to a fresh project path on next save
            }
          } else {
            // in-memory buffer being named for the first time → write it now
            try {
              const newPath = this.vfs.join(this.vfs.PROJECT || '/project', newName);
              this._savingToVFS = true;
              this.vfs.write(newPath, this.fileContents[newName] || '');
              this._savingToVFS = false;
              this.filePaths[newName] = newPath;
            } catch (e) { this._savingToVFS = false; }
          }
          // the tab still carries the old data-file here, so update its saved
          // indicator directly rather than via a name lookup
          tab.classList.toggle('is-unsaved', !this.filePaths[newName]);
        }
    ...
    _closeFile(name) {
      ...
      // forget this buffer's filesystem mapping (the file itself stays on disk)
      if (this.filePaths) delete this.filePaths[name];
      if (this.currentFile === name) clearTimeout(this._autosaveTimer);
    ...
    _newFile() {
      ...
      // A brand-new buffer lives ONLY in memory. It is not written to the
      // shared filesystem (so it won't show in the file browser, and can't be
      // orphaned by a delete there) until the user either renames the tab to
      // give it a name, or presses Run — both of which save it.
      this._openFile(name);
      this.textarea.focus();
    }
    ...
    // Open a VFS file in a (new or existing) tab and focus it.
    openInEditor(path) {
      if (!this.vfs) return;
      path = this.vfs.normalize(path);
      if (!this.vfs.isFile(path)) return;

      // already open? (same VFS path) → just focus it
      for (const tabName in this.filePaths) {
        if (this.filePaths[tabName] === path) {
          this._ensureTab(tabName);
          this._openFile(tabName);
      ...
      const name = this._dedupe(base, this.fileContents);
      this.fileContents[name] = this.vfs.read(path) || '';
      this.filePaths[name] = path;

      if (this._makeTab && this.tabsEl) {
        const tab = this._makeTab(name);
        const addBtn = this.tabsEl.querySelector('.se-tab-add');
        this.tabsEl.insertBefore(tab, addBtn || null);
      }
      this._openFile(name);
      this.textarea && this.textarea.focus();
    }
    ...
    _ensureTab(name) {
      ...
      const sel = '.se-tab[data-file="' + (window.CSS && CSS.escape ? CSS.escape(name) : name) + '"]';
      if (!this.tabsEl.querySelector(sel) && this._makeTab) {
        const tab = this._makeTab(name);
        const addBtn = this.tabsEl.querySelector('.se-tab-add');
        this.tabsEl.insertBefore(tab, addBtn || null);
      }
    ...
    // Write the current buffer back to the shared filesystem.
    // Returns the VFS path written, or null when there's no filesystem.
    _saveToVFS(silent) {
      if (!this.vfs) return null;
      if (this.textarea) this.fileContents[this.currentFile] = this.textarea.value;
      let path = this.filePaths[this.currentFile];
      if (!path) {
        // unmapped tab → save it into the project root under its tab name
        path = this.vfs.join(this.vfs.PROJECT || '/project', this.currentFile);
        this.filePaths[this.currentFile] = path;
      }
      this._savingToVFS = true;
      this.vfs.write(path, this.fileContents[this.currentFile] || '');
      this._savingToVFS = false;
      this._setTabSaved(this.currentFile, true);
      if (!silent) this._flashSaved(path);
      return path;
    }

    _flashSaved(path) {
      if (this.pathFile) {
        const prev = this.pathFile.textContent;
        this.pathFile.textContent = 'saved → ' + path;
        clearTimeout(this._savedT);
        this._savedT = setTimeout(() => { if (this.pathFile) this.pathFile.textContent = this.currentFile; }, 1400);
      }
    }
    ...
    // Persist edits to the shared filesystem — but ONLY for files that already
    // have a home there (i.e. already saved by naming or running). Untitled
    // in-memory buffers are deliberately left alone until named or run.
    _scheduleAutosave() {
      if (!this.vfs || !this.filePaths) return;
      if (!this.filePaths[this.currentFile]) return;
      clearTimeout(this._autosaveTimer);
      this._autosaveTimer = setTimeout(() => this._saveToVFS(true), 450);
    }
    ...
    _run() {
      ...
      // keep the in-RAM filesystem in sync so imports resolve against the
      // exact source the student is looking at
      if (this.vfs) { this.fileContents[this.currentFile] = code; this._saveToVFS(true); }
      ...
      // Try to use sage-repl evaluator if loaded — VFS-aware so `import sage-numpy`
      // resolves a package Myst installed into ./myst_libs.
      if (global.SageEvaluator) {
        const cwd = this.vfs ? (this.vfs.PROJECT || '/project') : '/project';
        const ev = new global.SageEvaluator(txt => add(txt), { cwd: cwd, vfs: this.vfs });
        try {
          ev.evalSource(code);
    ...
    // ── Unsaved-buffer helpers (used by the playground's close prompt) ──────
    // A buffer is "unsaved" when it has no home on the virtual filesystem yet
    // (mapped buffers autosave, so they're always in sync).
    _unsavedNames() {
      if (this.textarea && this.fileContents[this.currentFile] !== undefined) {
        this.fileContents[this.currentFile] = this.textarea.value;
      }
      return Object.keys(this.fileContents).filter(
        (n) => !(this.filePaths && this.filePaths[n])
      );
    }
    hasUnsaved() { return this._unsavedNames().length > 0; }
    unsavedCount() { return this._unsavedNames().length; }

    // Write every unsaved buffer into the shared filesystem. Returns the count.
    saveAllToVFS() {
      if (!this.vfs) return 0;
      const names = this._unsavedNames();
      names.forEach((n) => {
        const path = this.vfs.join(this.vfs.PROJECT || '/project', n);
        if (this.filePaths) this.filePaths[n] = path;
        this._savingToVFS = true;
        this.vfs.write(path, this.fileContents[n] || '');
        this._savingToVFS = false;
        this._setTabSaved(n, true);
      });
      return names.length;
    }

    // Offer every unsaved buffer as a file download. Returns the count.
    downloadUnsaved() {
      const names = this._unsavedNames();
      names.forEach((n) => {
        const blob = new Blob([this.fileContents[n] || ''], { type: 'text/plain;charset=utf-8' });
    ...
    // ── Teardown (called by the playground when an app is reset/remounted) ──
    destroy() {
      clearTimeout(this._autosaveTimer);
      clearTimeout(this._savedT);
      if (this._offVfs) { this._offVfs(); this._offVfs = null; }
      if (this._openListener) {
        global.removeEventListener('sage:open-file', this._openListener);
